...
| **C9** | Continuous integration | "There is no continuous integration. No CI configuration of any kind exists in the repository — no workflow directory, no workflow file, and the only YAML file in the tree is the Docker compose file. The repository is under version control at a single commit on `main` with a remote configured." | "`test_llm_gateway_cannot_write_state` guards in CI." "Guarded in CI." "Checks run on every change." — No CI exists to guard anything. Note also that the audit's phrasing "no CI, and no `.git` directory at all" is now half-stale: the no-CI half stands, the no-`.git` half does not. | **TARGET, not delivered:** real CI running only checks that genuinely exist and genuinely pass (`HANDOFF.md` P0.3). |
